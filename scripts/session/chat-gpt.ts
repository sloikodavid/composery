...
export async function readChatGptShare(url: string): Promise<Session> {
	...
	for (const node of shared.linear_conversation as { message?: Message }[]) {
		...
		if (text !== undefined) {
			const createdAt = node.message.create_time;
			items.push({
				kind: "turn",
				author: role,
				text,
				...(typeof createdAt === "number" && {
					timestamp: new Date(createdAt * millisecondsPerSecond),
				}),
			});
		}
	}
