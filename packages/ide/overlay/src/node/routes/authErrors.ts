 ...
 * password back to login, change-password sends a box with no password to
...
const MESSAGES: Record<AuthPage, Record<string, () => string>> = {
	login: {
		...
		"env-managed": () =>
			"COMPOSERY_PASSWORD is set on this box and takes precedence over a password set here. Change or remove that variable instead."
	},
	register: {
...
export const authErrorCodes = (page: AuthPage): string[] =>
	Object.keys(MESSAGES[page]);
