...
export const SESSION_LIFETIMES = {
	browser: { maxAgeSeconds: 30 * 24 * 60 * 60, persistent: false },
	"8h": { maxAgeSeconds: 8 * 60 * 60, persistent: true },
	"1d": { maxAgeSeconds: 24 * 60 * 60, persistent: true },
	"7d": { maxAgeSeconds: 7 * 24 * 60 * 60, persistent: true },
	"30d": { maxAgeSeconds: 30 * 24 * 60 * 60, persistent: true }
} as const;

export type SessionLifetime = keyof typeof SESSION_LIFETIMES;

export function readSessionLifetime(
	value = process.env.COMPOSERY_SESSION_LIFETIME
): SessionLifetime {
	...
	throw new Error(
		`COMPOSERY_SESSION_LIFETIME must be one of ${Object.keys(
			SESSION_LIFETIMES
		).join(", ")}`
	);
}
...
function signature(payload: string, value: string): string {
	return crypto
		.createHmac("sha256", signingKey(value))
		.update(payload)
		.digest("base64url");
}
...
export function createSessionToken(
	...
	const expiresAt = issuedAt + SESSION_LIFETIMES[lifetime].maxAgeSeconds;
...
export function isSessionTokenValid(
	...
	const match =
		/^v1\.(\d{1,12})\.(\d{1,12})\.([A-Za-z0-9_-]{43})\.([A-Za-z0-9_-]{43})$/.exec(
			token
		);
	...
	if (
		...
		expiresAt <= issuedAt ||
		expiresAt - issuedAt > SESSION_LIFETIMES["30d"].maxAgeSeconds
	) {
		return false;
	}
...
export function setSessionCookie(
	...
	res.cookie(
		req.cookieSessionName,
		createSessionToken(req.args),
		sessionCookieOptions(baseOptions)
	);
}
...
export function sessionCookieOptions(
	...
	if (policy.persistent) {
		options.maxAge = policy.maxAgeSeconds * 1000;
	}
	return options;
}
