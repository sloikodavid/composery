...
class Watcher {
	...
	private reloadWebServer = (): void => {
		...
		this.webServer = spawn("node", [
			path.join(this.rootPath, "out/node/entry.js"),
			...args
		]);
		onLine(this.webServer, (line) => console.log("[code-server]", line));
		const { pid } = this.webServer;

		this.webServer.on("exit", () =>
			console.log("[code-server]", `Web process ${pid} exited`)
		);
	...
	private readonly compilers: DevelopmentCompilers = {
		codeServer: spawn("tsc", ["--watch", "--pretty", "--preserveWatchOutput"], {
			cwd: this.rootPath
		}),
		vscode: spawn("npm", ["run", "watch"], { cwd: this.paths.vscodeDir }),
		vscodeWebExtensions: spawn("npm", ["run", "watch-web"], {
	...
	public async initialize(): Promise<void> {
		...
		for (const [processName, devProcess] of Object.entries(this.compilers)) {
			...
			if (devProcess.stderr) {
				devProcess.stderr.on("data", (d: string | Uint8Array) =>
	...
	private parseVSCodeLine: OnLineCallback = (strippedLine, originalLine) => {
		...
		if (strippedLine.includes("Finished compilation with")) {
			console.log(
				"[Code OSS] ✨ Finished compiling! ✨",
				"(Refresh your web browser ♻️)"
			);
			this.reloadWebServer();
		}
	};

	private parseCodeServerLine: OnLineCallback = (
		strippedLine,
		originalLine
	) => {
		if (!strippedLine.length) return;
