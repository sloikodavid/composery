...
describe("parser", () => {
	beforeEach(() => {
		...
		delete process.env.COMPOSERY_DISABLE_FILE_DOWNLOADS;
		delete process.env.COMPOSERY_DISABLE_GETTING_STARTED_OVERRIDE;
		delete process.env.COMPOSERY_RECONNECTION_GRACE_TIME;
		delete process.env.VSCODE_PROXY_URI;
		delete process.env.COMPOSERY_DISABLE_PROXY;
		console.log = jest.fn();
	});
	...
	it("should error if value isn't provided", () => {
		...
		expect(() => parse(["--auth=", "--log=debug"])).toThrowError(
			/--auth requires a value/
		);
		expect(() => parse(["--auth", "--log"])).toThrowError(
			/--auth requires a value/
		);
		expect(() => parse(["--auth", "--invalid"])).toThrowError(
			/--auth requires a value/
		);
		expect(() => parse(["--bind-addr"])).toThrowError(
			/--bind-addr requires a value/
		);
	});

	it("should error if value is invalid", () => {
		expect(() => parse(["--port", "foo"])).toThrowError(
			/--port must be a number/
		);
		expect(() => parse(["--auth", "invalid"])).toThrowError(
			/--auth valid values: \[password, none\]/
		);
		expect(() => parse(["--log", "invalid"])).toThrowError(
	...
	it("should not allow option-like values", () => {
		expect(() => parse(["--socket", "--socket-path-value"])).toThrowError(
		...
		expect(() => parse(["--cert", "--socket-path-value"])).toThrowError(
	...
	it("should support repeatable flags", async () => {
		expect(() => parse(["--proxy-domain", ""])).toThrowError(
			/--proxy-domain requires a value/
		);
		expect(parse(["--proxy-domain", "*.coder.com"])).toEqual({
			"proxy-domain": ["*.coder.com"]
		});
		expect(
			parse(["--proxy-domain", "*.coder.com", "--proxy-domain", "test.com"])
		).toEqual({
	...
	it("should enforce cert-key with cert value or otherwise generate one", async () => {
		...
		expect(() => parse(["--cert", "test"])).toThrowError(
			/--cert-key is missing/
		);
	...
	it("should use env var COMPOSERY_DISABLE_FILE_DOWNLOADS", async () => {
		process.env.COMPOSERY_DISABLE_FILE_DOWNLOADS = "1";
		const args = parse([]);
		expect(args).toEqual({});
	...
	it("should use env var COMPOSERY_DISABLE_FILE_DOWNLOADS set to true", async () => {
		process.env.COMPOSERY_DISABLE_FILE_DOWNLOADS = "true";
		const args = parse([]);
		expect(args).toEqual({});
	...
	it("should use env var COMPOSERY_DISABLE_GETTING_STARTED_OVERRIDE", async () => {
		process.env.COMPOSERY_DISABLE_GETTING_STARTED_OVERRIDE = "1";
		const args = parse([]);
		expect(args).toEqual({});
	...
	it("should use env var COMPOSERY_DISABLE_GETTING_STARTED_OVERRIDE set to true", async () => {
		process.env.COMPOSERY_DISABLE_GETTING_STARTED_OVERRIDE = "true";
		const args = parse([]);
		expect(args).toEqual({});
	...
	it("should use env var COMPOSERY_DISABLE_PROXY", async () => {
		process.env.COMPOSERY_DISABLE_PROXY = "1";
		const args = parse([]);
		expect(args).toEqual({});
	...
	it("should use env var COMPOSERY_DISABLE_PROXY set to true", async () => {
		process.env.COMPOSERY_DISABLE_PROXY = "true";
		const args = parse([]);
		expect(args).toEqual({});
	...
	it("should use env var COMPOSERY_RECONNECTION_GRACE_TIME for reconnection grace time", async () => {
		process.env.COMPOSERY_RECONNECTION_GRACE_TIME = "86400";
		const args = parse([]);
		expect(args).toEqual({});
	...
	it("should fail to parse invalid config", () => {
		expect(() => parseConfigFile("test", "/fake-config-path")).toThrowError(
			"invalid config: test"
		);
	});
...
describe("cli", () => {
	...
	it("should use existing if --reuse-window is set", async () => {
		...
		await expect(
			shouldOpenInExistingInstance(args, sessionSocket)
		).rejects.toThrow();
		...
		await expect(
			shouldOpenInExistingInstance(args, sessionSocket)
		).resolves.toStrictEqual(socketPath);
	...
	it("should use existing if --new-window is set", async () => {
		...
		expect(
			await shouldOpenInExistingInstance(args, sessionSocket)
		).toStrictEqual(socketPath);
	...
	it("should prefer matching sessions for only the first path", async () => {
		...
		expect(
			await shouldOpenInExistingInstance(args, sessionSocket)
		).toStrictEqual(`${tmpDirPath}/vscode-ipc-aaa.sock`);
...
describe("bindAddrFromArgs", () => {
	...
	it("should use process.env.COMPOSERY_HOST if set", () => {
		const [setValue, resetValue] = useEnv("COMPOSERY_HOST");
		setValue("coder");
	...
	it("should use the args.host over process.env.COMPOSERY_HOST if both set", () => {
		const [setValue, resetValue] = useEnv("COMPOSERY_HOST");
		setValue("coder");
...
describe("defaultConfigFile", () => {
	it("should return the default config file as a string", () => {
		const actual = defaultConfigFile();
...
describe("optionDescriptions", () => {
			...
			const exists = actualOptionDescriptions.find((desc) => {
				if (
					desc
						.replace(/\n/g, " ")
						.replace(/ /g, "")
						.includes(expectedDescription.replace(/\n/g, " ").replace(/ /g, ""))
				) {
					return true;
				}
				return false;
	...
	it("should visually align multiple options", () => {
		...
		expect(optionDescriptions(opts)).toStrictEqual([
			...
			`  --disable-update-check Disable update check. Without this flag, Composery checks every 6 hours against the latest GitHub release and
                          then notifies you once every week that a new release is available.`
		]);
	});
