...
describe("login", () => {
	...
	describe("/login", () => {
		...
		it("should return localized login title", async () => {
			process.env.PASSWORD = previousEnvPassword;
			const codeServer = await integration.setup(["--locale=zh-cn"], "");
			const resp = await codeServer.fetch("/login", { method: "GET" });

			const htmlContent = await resp.text();
			expect(resp.status).toBe(200);
			expect(htmlContent).toContain("<title>Composery 登录</title>");
		});
	});
});
