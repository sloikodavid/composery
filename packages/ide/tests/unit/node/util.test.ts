...
describe("getEnvPaths", () => {
	describe("on darwin", () => {
		...
		it("should return the env paths using xdgBasedir", () => {
			...
			expect(envPaths.data).toEqual("/home/usr/.local/share/composery");
			expect(envPaths.config).toEqual("/home/usr/.config/composery");
			expect(envPaths.runtime).toEqual("/tmp/runtime/composery");
		});
...
describe("isHashMatch", () => {
	...
	it("should return true with actual hash", async () => {
		const password = "password123";
		const _hash =
			"$argon2i$v=19$m=4096,t=3,p=1$EAoczTxVki21JDfIZpTUxg$rkXgyrW4RDGoDYrxBFD4H2DlSMEhP4h+Api1hXnGnFY";
		const actual = await util.isHashMatch(password, _hash);
		expect(actual).toBe(true);
	});
	...
	it("should reject the promise and throw if error", async () => {
		...
		const _hash = "$ar2i";
		expect(
			async () => await util.isHashMatch(password, _hash)
		).rejects.toThrow();
	});
});
...
describe("handlePasswordValidation", () => {
	it("should return true with a hashedPassword for a PLAIN_TEXT password", async () => {
		...
		const matchesHash = await util.isHashMatch(
			p,
	...
	it("should return true with a hashedPassword for a SHA256 password", async () => {
		...
		const matchesHash = util.isHashLegacyMatch(
			p,
			passwordValidation.hashedPassword
		);
...
describe("onLine", () => {
	...
	beforeAll(() => {
		proc = cp.spawn("node", [
			"-e",
			'process.stdin.setEncoding("utf8");process.stdin.on("data", console.log)'
		]);
	});
...
describe("open", () => {
	it("should throw an error if address is a string", async () => {
		...
		await expect(util.open(address)).rejects.toThrow(
...
describe("constructOpenOptions", () => {
	it("should return options for darwin", () => {
		...
		const { args, command, urlSearch } = util.constructOpenOptions(
			platform,
			url.search
		);
...
describe("splitOnFirstEquals", () => {
	...
	tests.forEach((test) => {
		it("should ${test.name}", () => {
			const input =
				test.key && typeof test.value !== "undefined"
					? `${test.key}=${test.value}`
					: test.key;
