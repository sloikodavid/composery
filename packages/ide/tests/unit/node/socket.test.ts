...
describe("SocketProxyProvider", () => {
	...
	beforeAll(async () => {
		...
		const socketPath = await provider.findFreeSocketPath(
			path.join(tmpdir, "tests/tls-socket-proxy")
		);
			...
			const main = tls
				.createServer(options, (s) => {
					...
					server
						...
						.on("error", (error) =>
							onServerError.emit({ event: "error", error })
						)
						.on("end", () =>
							onServerError.emit({
								event: "end",
						...
						.on("close", () =>
							onServerError.emit({
								event: "close",
								error: new Error("unexpected close")
							})
						);
					resolve("server");
				})
				...
				.listen(socketPath, () => {
					client = tls
						...
						.on("error", (error) =>
							onClientError.emit({ event: "error", error })
						)
						.on("end", () =>
							onClientError.emit({
								event: "end",
	...
	it("should work without a proxy", async () => {
		...
		const dataFromServerToClient = (
			await getData(fromServerToClient)
		).toString();
		expect(dataFromServerToClient).toBe("server->client");
		client.write("client->server");
		const dataFromClientToServer = (
			await getData(fromClientToServer)
		).toString();
	...
	it("should work with a proxy", async () => {
		...
		proxy = (await provider.createProxy(server))
			...
			.on("end", () =>
				onProxyError.emit({ event: "end", error: new Error("unexpected end") })
			)
			.on("close", () =>
				onProxyError.emit({
					event: "close",
