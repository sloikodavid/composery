...
describe("http", () => {
	...
	describe("isTrustedOrigin", () => {
		it("should match exact origins", () => {
			expect(http.isTrustedOrigin("localhost:8080", ["localhost:8080"])).toBe(
				true
			);
		...
		it("should match *.example.com wildcard (same style as --proxy-domain)", () => {
			expect(http.isTrustedOrigin("sub.example.com", ["*.example.com"])).toBe(
				true
			);
			expect(http.isTrustedOrigin("example.com", ["*.example.com"])).toBe(true);
			expect(http.isTrustedOrigin("evil.com", ["*.example.com"])).toBe(false);
			expect(
				http.isTrustedOrigin("example.com.evil.com", ["*.example.com"])
			).toBe(false);
		});
			...
			[
				...
				it(`${test.origin} -> [${key}: ${value}]`, () => {
					const req = getMockReq({
						...
						args: {
							"trusted-origins": (test as { trustedOrigins?: string[] })
								.trustedOrigins
						}
					});
	...
	describe("constructRedirectPath", () => {
		it("should preserve slashes in queryString so they are human-readable", () => {
			...
			const mockTo = "";
			const actual = http.constructRedirectPath(
				mockReq,
				mockQueryParams,
				mockTo
			);
