...
describe("createApp", () => {
	...
	it("should handle error events on the server", async () => {
		...
		expect(logger.error).toHaveBeenCalledWith(
			`http server error: ${testError.message} ${testError.stack}`
		);
	...
	itUnix(
		"should reject errors that happen before the server can listen",
		async () => {
			...
			expect(() => masterBall()).rejects.toThrow(
				"listen EACCES: permission denied"
			);
		}
	);

	itUnix("should unlink a socket before listening on the socket", async () => {
	...
	itUnix("should change the file mode of a socket", async () => {
		const defaultArgs = await setDefaults({
			socket: tmpFilePath,
			"socket-mode": "777"
		});
...
describe("ensureAddress", () => {
	...
	it("should throw and error if no address", () => {
		expect(() => ensureAddress(mockServer, "http")).toThrow(
...
describe("listen", () => {
	...
	itUnix(
		"should throw an error if a directory is passed in instead of a file",
		async () => {
