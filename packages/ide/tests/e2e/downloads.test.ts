...
describe("Downloads (enabled)", ["--disable-workspace-trust"], {}, async () => {
	...
	test("should see the 'Show Local' button on Save As", async ({
		codeServerPage
	}) => {
		// Setup
		const workspaceDir = await codeServerPage.workspaceDir;
	...
	test("should see the 'Show Local' button on Save File", async ({
		codeServerPage
	}) => {
		// Action
	...
	test("should see the 'Show Local' button on Save Workspace As", async ({
		codeServerPage
	}) => {
		// Action
...
describe(
	"Downloads (disabled)",
	["--disable-workspace-trust", "--disable-file-downloads"],
	{},
	async () => {
		...
		test("should not see the 'Download...' option", async ({
			codeServerPage
		}) => {
			// Setup
			const workspaceDir = await codeServerPage.workspaceDir;
			...
			expect(await codeServerPage.page.isVisible("text=Download...")).toBe(
				false
			);
		});

		test("should not see the 'Show Local' button on Save as", async ({
			codeServerPage
		}) => {
			// Setup
			const workspaceDir = await codeServerPage.workspaceDir;
			...
			await codeServerPage.page.click(".tab");
			await codeServerPage.navigateMenus(["File", "Save As..."]);
			expect(await codeServerPage.page.isVisible("text=Show Local")).toBe(
				false
			);
		});

		test("should not see the 'Show Local' button on Save File", async ({
			codeServerPage
		}) => {
			// Action
			await codeServerPage.navigateMenus(["File", "New Text File"]);
		...
		test("should not see the 'Show Local' button on Save Workspace As", async ({
			codeServerPage
		}) => {
			// Action
			await codeServerPage.navigateMenus(["File", "Save Workspace As..."]);
