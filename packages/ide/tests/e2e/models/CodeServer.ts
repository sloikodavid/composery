...
export class CodeServer {
	...
	constructor(
		...
		private readonly entry = process.env.COMPOSERY_TEST_ENTRY || "."
	) {
	...
	private async createWorkspace(): Promise<string> {
		...
		await fs.writeFile(
			...
			JSON.stringify(languagepacksContent)
		);
		return dir;
	}
	...
	private async spawn(): Promise<CodeServerProcess> {
		...
		return new Promise((resolve, reject) => {
			...
			const timer = idleTimer(
				"Failed to extract address; did the format change?",
				reject
			);
			...
			onLine(proc, (line) => {
				...
				let match = line
					.trim()
					.match(/HTTPS? server listening on (https?:\/\/[.:\d]+)\/?$/);
				if (match) {
				...
				if (
					typeof httpAddress !== "undefined" &&
					typeof sessionAddress !== "undefined"
				) {
					resolved = true;
					timer.dispose();
					this.logger.debug(
						`code-server is ready: ${httpAddress} ${sessionAddress}`
					);
...
export class CodeServerPage {
	...
	async reloadUntilEditorIsReady() {
		...
		while (!editorIsVisible) {
			...
			if (await this.isEditorVisible()) {
				this.codeServer.logger.debug(
					`editor became ready after ${reloadCount} reloads`
				);
				break;
			}
	...
	async isEditorVisible() {
		...
		this.codeServer.logger.debug(
			`Editor is ${visible ? "not visible" : "visible"}!`
		);
	...
	async focusTerminal() {
		...
		while (!(await doFocus())) {
			++attempts;
			this.codeServer.logger.debug(
				`no focused terminal textarea, retrying (${attempts}/∞)`
			);
		}

		this.codeServer.logger.debug(
			`opening terminal took ${attempts} ${plural(attempts, "attempt")}`
		);
	}
	...
	async navigateItems(
		...
		const openThenWaitClose = async (ctx: Context) => {
			...
			try {
				...
				if (!ctx.finished()) {
					this.codeServer.logger.debug(
						`${selector} navigation: ${(error as any).message || error}`
					);
				}
			}
			return false;
		...
		const navigate = async (ctx: Context) => {
			...
			for (const item of items) {
				...
				steps.push({
					fn: () =>
						this.page.hover(`${selector} :text-is("${item}")`, { trial: true }),
					name: `${item}:hover:trial`
				});
				steps.push({
					fn: () =>
						this.page.hover(`${selector} :text-is("${item}")`, { force: true }),
					name: `${item}:hover:force`
				});
				steps.push({
					fn: () =>
						this.page.click(`${selector} :text-is("${item}")`, { trial: true }),
					name: `${item}:click:trial`
				});
				steps.push({
					fn: () =>
						this.page.click(`${selector} :text-is("${item}")`, { force: true }),
		...
		while (
			!(await Promise.race([openThenWaitClose(context), navigate(context)]))
		) {
			++attempts;
	...
	async installExtension(id: string): Promise<void> {
		...
		await util.promisify(cp.exec)(
			`node . --install-extension ${id} --extensions-dir ${dir}`,
			{
				cwd: path.join(__dirname, "../../..")
			}
		);
