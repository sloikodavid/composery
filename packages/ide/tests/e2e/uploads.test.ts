...
describe("Uploads (enabled)", ["--disable-workspace-trust"], {}, () => {
	...
	test("should see the 'Show Local' button on Open File", async ({
		codeServerPage
	}) => {
		// Action
...
describe(
	"Uploads (disabled)",
	["--disable-workspace-trust", "--disable-file-uploads"],
	{},
	() => {
		...
		test("should not see the 'Upload...' option", async ({
			codeServerPage
		}) => {
			// Setup
			const workspaceDir = await codeServerPage.workspaceDir;
		...
		test("should not see the 'Show Local' button on Open File", async ({
			codeServerPage
		}) => {
			// Action
			await codeServerPage.navigateMenus(["File", "Open File..."]);
