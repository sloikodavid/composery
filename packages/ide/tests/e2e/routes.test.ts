		...
		for (const [route, matchers] of Object.entries(routes)) {
			...
			for (const element of elements) {
				...
				if (src && !matchers.some((m) => m.test(src))) {
					throw new Error(
						`${src} did not match any validators for route ${route}`
					);
				}
			}
		}
...
describe(
	"VS Code Routes with code-workspace",
	["--disable-workspace-trust", CODE_WORKSPACE_DIR],
	{},
	async () => {
		test("should redirect to the passed in workspace using human-readable query", async ({
			codeServerPage
		}) => {
			const url = new URL(codeServerPage.page.url());
			const pathname = getMaybeProxiedPathname(url);
...
describe(
	"VS Code Routes with code-workspace",
	["--disable-workspace-trust", CODE_FOLDER_DIR],
	{},
	async () => {
		test("should redirect to the passed in folder using human-readable query", async ({
			codeServerPage
		}) => {
			const url = new URL(codeServerPage.page.url());
			const pathname = getMaybeProxiedPathname(url);
...
describe(
	...
	{},
	async () => {
		test("should redirect to last query folder/workspace", async ({
			codeServerPage
		}) => {
			const folder = process.env.CODE_FOLDER_DIR;
			const workspace = process.env.CODE_WORKSPACE_DIR;
			await codeServerPage.navigate(
				`/?folder=${folder}&workspace=${workspace}`
			);
