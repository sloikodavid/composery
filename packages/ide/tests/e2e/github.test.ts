...
if (process.env.GITHUB_TOKEN) {
	describe("GitHub token", ["--disable-workspace-trust"], {}, () => {
		test("should be logged in to pull requests extension", async ({
			codeServerPage
		}) => {
			await codeServerPage.exec("git init");
			await codeServerPage.exec(
				"git remote add origin https://github.com/coder/code-server"
			);
			await codeServerPage.installExtension(
			...
			// pre-injected.
			expect(
				await codeServerPage.page.isVisible("text=Select an account")
			).toBe(false);
		});
	});
	...
	describe(
		...
		{ GITHUB_TOKEN: "" },
		() => {
			test("should not be logged in to pull requests extension", async ({
				codeServerPage
			}) => {
				await codeServerPage.exec("git init");
				await codeServerPage.exec(
				...
				expect(
					await codeServerPage.page.isVisible(
