...
import { toNativeSocketPath } from "../../src/node/util";
...
export function useEnv(
	key: string
): [(nextValue: string | undefined) => string | undefined, () => void] {
 ...
 */
export async function getMaybeProxiedCodeServer(
...
export function getMaybeProxiedPathname(url: URL): string {
	if (process.env.USE_PROXY === "1") {
		...
		const pathWithoutProxy = url.pathname.split(
...
export function listenOn(...socketPaths: string[]): FakeVscodeSockets {
	...
	const servers = socketPaths.map((socketPath) => {
		...
		server.listen(toNativeSocketPath(socketPath));
		return server;
	});
