		...
		return new Promise((resolve, reject) => {
			const request = (uri: string): void => {
				...
				const client = httpx.get(
					uri,
					{ headers: { "User-Agent": "code-server" }, agent },
					(response) => {
						if (
							!response.statusCode ||
							...
							response.destroy();
							return reject(
								new Error(`${uri}: ${response.statusCode || "500"}`)
							);
						}
						...
						if (response.statusCode >= 300) {
							...
							if (!response.headers.location) {
								return reject(
									new Error("received redirect with no location header")
								);
							}
