...
export const listen = async (server: http.Server, opts: ListenOptions) => {
	const nativeSocket = isSocketOpts(opts)
		? toNativeSocketPath(opts.socket)
		: undefined;
	const isNamedPipe = nativeSocket
		? nativeSocket.startsWith("\\\\.\\pipe\\")
		: false;
...
export const createApp = async (args: DefaultedArgs): Promise<App> => {
	...
	const editorSessionManagerServer = await makeEditorSessionManagerServer(
		args["session-socket"],
		editorSessionManager
	);
	const disposeEditorSessionManagerServer = disposer(
