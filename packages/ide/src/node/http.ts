...
export const replaceTemplates = <T extends object>(
	...
	return content
		.replace(
			/{{TO}}/g,
			(typeof req.query.to === "string" && escapeHtml(req.query.to)) || "/"
		)
		.replace(/{{BASE}}/g, serverOptions.base)
		.replace(/{{COMPOSERY_STATIC_BASE}}/g, serverOptions.csStaticBase)
 ...
 */
export const ensureProxyEnabled = (
	req: express.Request,
	_?: express.Response,
...
export const authenticated = async (req: express.Request): Promise<boolean> => {
	switch (req.args.auth) {
		...
		case AuthType.Password: {
			...
			if (!req.args.password && !hashedPasswordFromArgs) {
				return false;
			}
 ...
 */
export const constructRedirectPath = (
	req: express.Request,
	query: qs.ParsedQs,
	to: string
): string => {
	const relativePath = normalize(
		`${relativeRoot(req.originalUrl)}/${to}`,
		true
	);
...
export const self = (req: express.Request): string => {
	return normalize(
		`${req.baseUrl}${req.originalUrl.endsWith("/") ? "/" : ""}`,
		true
	);
};
...
export function ensureOrigin(
	...
	try {
		...
		logger.debug(
			`${error instanceof Error ? error.message : error}; blocking request to ${req.originalUrl}`
		);
		throw new HttpError("Forbidden", HttpCode.Forbidden);
	}
}
...
export function authenticateOrigin(req: express.Request): void {
	...
	if (typeof host === "undefined") {
		...
		logger.error(
			"Are you behind a reverse proxy that does not forward the host?"
		);
