...
import { canConnect, toNativeSocketPath } from "./util";
...
export async function makeEditorSessionManagerServer(
	...
	router.get<
		{},
		GetSessionResponse | string | unknown,
		undefined,
		{ filePath?: string }
	>("/session", async (req, res) => {
		...
		const socketPath =
			await editorSessionManager.getConnectedSocketPath(filePath);
		const response: GetSessionResponse = { socketPath };
		res.json(response);
	});

	router.post<{}, string, AddSessionRequest | undefined>(
		"/add-session",
		async (req, res) => {
	...
	router.post<{}, string, DeleteSessionRequest | undefined>(
		"/delete-session",
		async (req, res) => {
	...
	getCandidatesForFile(filePath: string): EditorSessionEntry[] {
		...
		const checkMatch = (entry: EditorSessionEntry): boolean => {
			...
			const result = entry.workspace.folders.some((folder) =>
				filePath.startsWith(folder.uri.path + "/")
			);
...
export class EditorSessionManagerClient {
	...
	async getConnectedSocketPath(filePath: string): Promise<string | undefined> {
		const response = await new Promise<GetSessionResponse>(
			(resolve, reject) => {
				const opts = {
					path: "/session?filePath=" + encodeURIComponent(filePath),
					socketPath: toNativeSocketPath(this.codeServerSocketPath),
					method: "GET"
				};
