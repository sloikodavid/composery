...
export const generateCertificate = async (
	...
	try {
		...
		const certs = await selfsigned.generate(
		...
		await Promise.all([
			fs.writeFile(certPath, certs.cert),
			fs.writeFile(certKeyPath, certs.private)
		]);
	}
...
export function toNativeSocketPath(socketPath: string): string {
	...
	return path.join(
		"\\\\.",
		"pipe",
		"composery-" + crypto.createHash("sha256").update(socketPath).digest("hex")
	);
}
 ...
 */
export function getPasswordMethod(
	...
	switch (passwordMethod) {
		case "PLAIN_TEXT": {
			const isValid = passwordFromArgs
				? safeCompare(passwordFromRequestBody, passwordFromArgs)
				: false;
		...
		case "SHA256": {
			const isValid = isHashLegacyMatch(
				passwordFromRequestBody,
				hashedPasswordFromArgs || ""
			);
			passwordValidation.isPasswordValid = isValid;
			...
			passwordValidation.hashedPassword =
				hashedPasswordFromArgs || (await hashLegacy(passwordFromRequestBody));
			break;
		}
		case "ARGON2": {
			const isValid = await isHashMatch(
				passwordFromRequestBody,
				hashedPasswordFromArgs || ""
			);
			passwordValidation.isPasswordValid = isValid;
...
export const isWsl = async (
	...
	try {
		return (await fs.readFile(procVersionFilePath, "utf8"))
			.toLowerCase()
			.includes("microsoft");
	} catch (_) {
		return false;
 ...
 */
export function constructOpenOptions(
	platform: NodeJS.Platform | "wsl",
	urlSearch: string
): OpenOptions {
	const args: string[] = [];
...
export const open = async (address: URL | string): Promise<void> => {
	...
	const platform = (await isWsl(
		...
		? "wsl"
		: process.platform;
	const { command, args, urlSearch } = constructOpenOptions(
		platform,
		url.search
	);
...
export function isNodeJSErrnoException(
	...
	return (
		error !== undefined && (error as NodeJS.ErrnoException).code !== undefined
	);
}

// TODO: Replace with proper templating system.
export const escapeJSON = (value: cp.Serializable) =>
