...
export const runCodeCli = async (args: DefaultedArgs): Promise<void> => {
	...
	try {
		...
		process.env.COMPOSERY_PARENT_PID = process.pid.toString();
...
export const openInExistingInstance = async (
	args: DefaultedArgs,
	socketPath: string
): Promise<void> => {
	...
	const vscode = http.request(
		{
			path: "/",
			method: "POST",
			socketPath: toNativeSocketPath(socketPath)
		},
		(response) => {
			response.on("data", (message) => {
				logger.debug(
					"got message from Code",
					field("message", message.toString())
				);
			});
		}
	);
...
export const runCodeServer = async (
	...
	if (typeof serverAddress !== "string") {
		logger.info(
			`Open locally: ${toLocalBrowserAddress(serverAddress).toString()}`
		);
	}
	if (args.auth === AuthType.Password) {
		...
		} else if (!args.password && !args["hashed-password"]) {
			logger.info("    - No password configured yet");
	...
	if (args["idle-timeout-seconds"]) {
		logger.info(
			`  - Idle timeout set to ${args["idle-timeout-seconds"]} seconds`
		);
		...
		const startIdleShutdownTimer = () => {
			idleShutdownTimer = setTimeout(() => {
				logger.warn(
					`Idle timeout of ${args["idle-timeout-seconds"]} seconds exceeded`
				);
				wrapper.exit(0);
	...
	if (args["disable-proxy"]) {
		...
		logger.info(
			`  - ${plural(args["proxy-domain"].length, "Proxying the following domain")}:`
		);
	...
	if (process.env.VSCODE_PROXY_URI) {
		logger.info(
			`Using proxy URI in PORTS tab: ${process.env.VSCODE_PROXY_URI}`
		);
	}
	...
	if (sessionServerAddress) {
		logger.info(
			`Session server listening on ${sessionServerAddress.toString()}`
		);
	}
	...
	if (args.open) {
		try {
			...
			logger.error(
				"Failed to open",
				field("address", serverAddress.toString()),
				field("error", error)
			);
		}
	}
