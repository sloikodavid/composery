...
export const options: Options<Required<UserProvidedArgs>> = {
	...
	"link-protection-trusted-domains": {
		type: "string[]",
		description:
			"Links matching a trusted domain can be opened without link protection."
	},
	locale: {
	...
	config: {
		type: "string",
		description:
			"Path to yaml config file. Every flag maps directly to a key in the config file."
	},
	...
	"skip-auth-preflight": {
		type: "boolean",
		description:
			"Allows preflight requests through proxy without authentication."
	},
	"ignore-last-opened": {
		type: "boolean",
		short: "e",
		description:
			"Ignore the last opened directory or workspace in favor of an empty window."
	},
	"new-window": {
		type: "boolean",
		short: "n",
	...
	i18n: {
		...
		path: true,
		description:
			"Path to JSON file with custom translations. Merges with default strings and supports all i18n keys."
	},
...
export const optionDescriptions = (
	...
	const widths = entries.reduce(
		(prev, [k, v]) => ({
			...
			short:
				v.short && v.short.length > prev.short ? v.short.length : prev.short
		}),
		{ short: 0, long: 0 }
	);
	...
	for (let i = 0; i < argv.length; ++i) {
		...
		if (!ended && arg.startsWith("-")) {
			...
			if (typeof value === "undefined") {
				...
				value =
					argv[i + 1] && !argv[i + 1].startsWith("-") ? argv[++i] : undefined;
			}
			...
			switch (option.type) {
				...
				default: {
					if (!Object.values(option.type).includes(value)) {
						throw error(
							`--${key} valid values: [${Object.values(option.type).join(", ")}]`
						);
					}
					(args[key] as string) = value;
					break;
...
export async function setDefaults(
	cliArgs: UserProvidedArgs,
	configArgs?: ConfigArgs
): Promise<DefaultedArgs> {
	...
	if (
		process.env.COMPOSERY_DISABLE_GETTING_STARTED_OVERRIDE?.match(/^(1|true)$/)
	) {
	...
	if (process.env.COMPOSERY_RECONNECTION_GRACE_TIME) {
		args["reconnection-grace-time"] =
			process.env.COMPOSERY_RECONNECTION_GRACE_TIME;
	}
	...
	if (process.env.COMPOSERY_IDLE_TIMEOUT_SECONDS) {
		...
		args["idle-timeout-seconds"] = Number(
			process.env.COMPOSERY_IDLE_TIMEOUT_SECONDS
		);
	}
	...
	const proxyDomains = new Set(
		(args["proxy-domain"] || []).map((d) => d.replace(/^\*\./, ""))
	);
	const finalProxies = [];
...
export const toCodeArgs = async (args: DefaultedArgs): Promise<CodeArgs> => {
	return {
		...
		"reset-password":
			args.auth === AuthType.Password &&
			!args.usingEnvPassword &&
			!args.usingEnvHashedPassword
	};
};
