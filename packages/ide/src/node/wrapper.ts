...
abstract class Process {
	...
	public preventExit(): void {
		(process.exit as any) = (code?: number) => {
			this.logger.warn(
				`process.exit() was prevented: ${code || "unknown code"}.`
			);
		};
	}
...
export class ParentProcess extends Process {
	...
	public constructor(private currentVersion: string) {
		...
		this.onChildMessage((message) => {
			switch (message.type) {
				case "relaunch":
					this.logger.info(
						`Relaunching: ${this.currentVersion} -> ${message.version}`
					);
	...
	private spawn(): cp.ChildProcess {
		return cp.fork(path.join(__dirname, "entry"), {
			env: {
				...process.env,
				COMPOSERY_PARENT_PID: process.pid.toString(),
...
export const wrapper =
	typeof process.env.COMPOSERY_PARENT_PID !== "undefined"
		? new ChildProcess(parseInt(process.env.COMPOSERY_PARENT_PID))
