...
} from "@/convex/fleet/operationSweep";
import { workflow } from "@/convex/fleet/workflows/boxWorkflow";
import {
	readBox,
	readOperation,
	seedBox,
...
describe("the stuck-operation sweep", () => {
	...
	test("closes an operation that never recorded a workflow", async () => {
		...
		await t.action(internal.fleet.operationSweep.sweepStuckOperations, {});
	...
	test("reports a long-running live operation to staff instead of closing it", async () => {
		...
		await t.mutation(internal.fleet.operationSweep.alertLongRunningOperation, {
			boxId,
	...
	test("lists only operations older than the cutoff it was given", async () => {
		...
		const listed = await t.query(
			internal.fleet.operationSweep.activeOperationsBefore,
			{ before: 5_000 }
		);
...
describe("what the rescue refuses to touch", () => {
	...
	test.each(["succeeded", "failed"] as const)(
		...
		async (status) => {
			...
			await t.mutation(internal.fleet.operationSweep.failOrphanedOperation, {
