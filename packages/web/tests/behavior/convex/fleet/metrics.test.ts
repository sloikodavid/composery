...
describe("the repeat-flag cooloff", () => {
	...
	async function pollOverThreshold(t: Harness, boxId: Id<"boxes">) {
		for (let index = 0; index < 6; index += 1) {
			await t.mutation(internal.fleet.metrics.recordSample, {
				boxId,
				cpuPercent: 1,
				ingressBps: 1,
...
describe("flagging a box from its samples", () => {
	...
	async function poll(
		...
		return await t.mutation(internal.fleet.metrics.recordSample, {
			boxId,
			cpuPercent: 1,
			ingressBps: 1,
...
describe("rolling samples up into hours", () => {
	...
	async function rollup(t: Harness) {
		await t.mutation(internal.fleet.metrics.rollupHourlyMetrics, {});
...
describe("deleting metrics past their retention", () => {
	...
	test("removes raw samples and hourly rows past their own windows", async () => {
		...
		await t.mutation(internal.fleet.metrics.deleteOldSamples, {});
	...
	test("comes back for the rest when it fills a batch", async () => {
		...
		expect(
			await scheduledJobs(t, "fleet/metrics:deleteOldSamples")
		).toHaveLength(1);
	});
	...
	test("stops when it did not fill a batch", async () => {
		...
		expect(await scheduledJobs(t, "fleet/metrics:deleteOldSamples")).toEqual(
			[]
		);
	});
});
...
describe("walking the fleet to roll its metrics up", () => {
	const rollup = (t: Harness, args: object = {}) =>
		t.mutation(internal.fleet.metrics.rollupHourlyMetrics, args);
	...
	test("moves on to the next status when a page is finished", async () => {
		...
		expect(
			await scheduledArgs<{ statusIndex: number }>(
				t,
				"fleet/metrics:rollupHourlyMetrics"
			)
	...
	test("stops once it has walked every status", async () => {
		...
		expect(await scheduledJobs(t, "fleet/metrics:rollupHourlyMetrics")).toEqual(
			[]
		);
	});
	...
	test("comes back for the boxes past the first page", async () => {
		...
		const next = (
			await scheduledArgs<{
				...
				hourStart: number;
				statusIndex: number;
			}>(t, "fleet/metrics:rollupHourlyMetrics")
		).find((job) => job.cursor);
...
describe("what a flag tells staff", () => {
	...
	const poll = (t: Harness, boxId: Id<"boxes">, both = false) =>
		t.mutation(internal.fleet.metrics.recordSample, {
			boxId,
			cpuPercent: 1,
			ingressBps: 1,
