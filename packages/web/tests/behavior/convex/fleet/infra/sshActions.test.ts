...
vi.mock("@/convex/fleet/infra/sshTransport", () => ({
	runSsh: vi.fn(
		async (
...
describe("writing a box's runtime files", () => {
	test("uses the recorded image for repair and the requested image for update", async () => {
		...
		await t.action(internal.fleet.infra.ssh.repairRuntime, { boxId });
		...
		ssh.calls.length = 0;
		await t.action(internal.fleet.infra.ssh.updateRuntime, {
			boxId,
	...
	test("bootstraps without waiting for the workflow's readiness check", async () => {
		...
		await t.action(internal.fleet.infra.ssh.bootstrapRuntime, { boxId });
	...
	test("applies the supplied configuration", async () => {
		...
		await t.action(internal.fleet.infra.ssh.applyRuntimeConfig, {
			boxId,
	...
	test("rewrites the complete environment with the new password", async () => {
		...
		await t.action(internal.fleet.infra.ssh.rewritePasswordAndRestart, {
			boxId,
	...
	test("reloads Caddy on the requested slug", async () => {
		...
		await t.action(internal.fleet.infra.ssh.reloadSlug, {
			boxId,
			newSlug: "renamed"
		});
...
describe("inspecting and reading a box", () => {
	const inspect = (t: Harness, boxId: Id<"boxes">) =>
		t.action(internal.fleet.infra.ssh.inspectRuntime, { boxId });
	...
	test("returns logs and clamps their requested tail", async () => {
		...
		await expect(
			t.action(internal.fleet.infra.ssh.fetchRuntimeLogs, {
				boxId,
				tail: 1_000_000
			})
...
describe("parking a box's files", () => {
	test("measures usage and rejects output without a measurement", async () => {
		...
		await expect(
			t.action(internal.fleet.infra.ssh.measureParkingUsage, { boxId })
	...
	test("stops on a copy difference and names its direction", async () => {
		...
		await expect(
			t.action(internal.fleet.infra.ssh.verifyParkingCopy, {
				boxId,
				volumeId: 909
			})
		).rejects.toThrow("verification (out) found 2 difference(s)");
		await expect(
			t.action(internal.fleet.infra.ssh.verifyParkingBack, {
				boxId,
				volumeId: 909
			})
	...
	test("reports the SSH reason when repair cannot reach the host", async () => {
		...
		await expect(
			t.action(internal.fleet.infra.ssh.requireReachableHost, { boxId })
...
describe("SSH command policy", () => {
	test.each([
		...
		async (action, args, maxOutputBytes, timeoutMs, stdout) => {
			...
			const boxId = await box(t);
			ssh.stdout = stdout;
			await t.action(internal.fleet.infra.ssh[action], {
				boxId,
				...args
			} as never);
...
describe("an action against a box with no host", () => {
	test.each([
		...
		async (action, args) => {
			...
			await expect(
				t.action(internal.fleet.infra.ssh[action], {
					boxId,
					...args
				} as never)
