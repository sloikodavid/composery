...
import { ownerCanReadBox } from "@/convex/fleet/queries";
...
describe("resolving a box by subscription", () => {
	test("finds the live box a subscription pays for", async () => {
		...
		expect(
			await t.query(internal.fleet.queries.boxIdBySubscription, {
...
describe("the internal box lookups", () => {
	test("resolves a slug for a caller that holds no session", async () => {
		...
		expect(
			await t.query(internal.fleet.queries.boxBySlug, { slug: "mine" })
	...
	test("resolves an owner's slug and refuses a stranger's", async () => {
		...
		expect(
			await t.query(internal.fleet.queries.boxByOwnerSlug, {
				slug: "mine",
	...
	test("hands a workflow its box, and throws when there is none", async () => {
		...
		expect(
			await t.query(internal.fleet.queries.getBoxLifecycleSnapshot, { boxId })
		).toMatchObject({ _id: boxId });

		await t.run(async (ctx) => await ctx.db.delete(boxId));
		await expect(
			t.query(internal.fleet.queries.getBoxLifecycleSnapshot, { boxId })
	...
	test("pages one owner's boxes in one status", async () => {
		...
		const page = await t.query(internal.fleet.queries.boxesForUserStatusPage, {
