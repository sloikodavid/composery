...
import { FLOOR_UPDATE_STATUSES } from "@/convex/fleet/runtimeFloor";
...
function pastDeadline(t: Harness) {
	return t.query(internal.fleet.runtimeFloor.boxesPastFloorDeadline, {});
}
...
describe("updating boxes past the floor deadline", () => {
	test("starts one update per selected box, recording why", async () => {
		...
		await t.action(internal.fleet.runtimeFloor.updateBoxesPastDeadline, {});
