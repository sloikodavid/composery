...
describe("OPERATION_ALLOWED_STATUSES", () => {
	...
	test("gives every operation at least one status it can start from", () => {
		for (const type of EVERY_OPERATION) {
			expect(OPERATION_ALLOWED_STATUSES[type].length).toBeGreaterThan(0);
		}
	});

	test("never references an unknown box status", () => {
	...
	test("never allows operating on an already-deleted box", () => {
...
describe("isOperationAllowed (state-machine transitions)", () => {
	test("starts a running box only from stopped", () => {
	...
	test("stops a running box only while running", () => {
	...
	test("allows deleting a box that is mid-repair or repair_failed", () => {
	...
	test("allows deleting from every live state except deleting/deleted", () => {
