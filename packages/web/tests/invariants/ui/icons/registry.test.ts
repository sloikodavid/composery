...
const ICONS_DIR = join(ROOT, "ui/icons");
const REGISTRY = join(ROOT, "ui/animated-icon.tsx");
...
function sourceFiles() {
	...
	for (const dir of ["app", "ui"]) walk(dir);
	return found;
}
...
describe("animated icon registry", () => {
	...
	test("is the only importer of a glyph module", () => {
		const offenders = sourceFiles().filter((file) => {
			if (file.startsWith("ui/icons/")) return false;
			if (file === "ui/animated-icon.tsx") return false;
