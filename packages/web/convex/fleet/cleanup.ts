...
export const finishFailedDeletions = internalAction({
	...
	handler: async (ctx) => {
		const pending = await ctx.runQuery(
			internal.fleet.cleanup.boxesFailedToDelete,
			{}
		);

		for (const box of pending) {
			if (deleteNeedsPerson(box.failedDeletes)) {
				await ctx.runMutation(internal.fleet.cleanup.alertDeletionNeedsPerson, {
					boxId: box.boxId,
	...
	handler: async (ctx) => {
		...
		for (const box of boxes) {
			...
			await ctx.scheduler.runAfter(
				0,
				internal.fleet.cleanup.deleteRuntimeData,
				{
					boxId: box._id
				}
			);
			await ctx.scheduler.runAfter(
				0,
				internal.fleet.cleanup.sanitizeOperations,
				{
					boxId: box._id
				}
			);
			await ctx.scheduler.runAfter(0, internal.fleet.cleanup.sanitizeEvents, {
				boxId: box._id
			});
			await ctx.scheduler.runAfter(
				0,
				internal.fleet.cleanup.startCheckoutRetention,
				{
					boxId: box._id,
					deletedAt
				}
...
export const scheduleExpiredBoxPurges = internalMutation({
	...
	handler: async (ctx, args) => {
		...
		for (const box of page.page) {
			await ctx.scheduler.runAfter(0, internal.fleet.cleanup.purgeBox, {
				boxId: box._id
			});
		}
		if (!page.isDone) {
	...
	handler: async (ctx, args) => {
		...
		if (rows.length > 0 || intents.length > 0 || snapshots.length > 0) {
			await ctx.scheduler.runAfter(
				...
				internal.fleet.cleanup.purgeBox,
				{ boxId: args.boxId }
			);
			return;
		}
