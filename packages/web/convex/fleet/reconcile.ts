	...
	handler: async (ctx) => {
		try {
			...
			const images = await ctx.runAction(
				internal.fleet.infra.hetznerVps.listProductSnapshotImages,
				{}
			);
			let deletedImages = 0;
			for (const image of images) {
				const known = await ctx.runQuery(
					internal.fleet.reconcile.snapshotImageIsKnown,
					{ imageId: image.imageId }
				);
				if (!isReclaimable(image.createdAtMs, now, known)) continue;
				await ctx.runAction(internal.fleet.infra.hetznerVps.deleteImage, {
					imageId: image.imageId
				});
				deletedImages += 1;
			}
			...
			const volumes = await ctx.runAction(
				internal.fleet.infra.hetznerVps.listProductVolumes,
				{}
			);
			let deletedVolumes = 0;
			for (const volume of volumes) {
				const referenced = await ctx.runQuery(
					internal.fleet.reconcile.volumeReferencedByLiveBox,
			...
			const servers = await ctx.runAction(
				internal.fleet.infra.hetznerVps.listProductServers,
				{}
			);
			const orphanedServers: { serverId: number; name?: string }[] = [];
			for (const server of servers) {
				const live = await ctx.runQuery(
					internal.fleet.reconcile.serverHasLiveBox,
			...
			const primaryIps = await ctx.runAction(
				internal.fleet.infra.hetznerVps.listUnassignedPrimaryIps,
				{}
			);
			...
			if (orphanedServers.length > 0) {
				...
				await ctx.runMutation(internal.fleet.reconcile.alertOrphanedServers, {
			...
			if (leakedPrimaryIps.length > 0) {
				...
				await ctx.runMutation(
					internal.fleet.reconcile.alertUnassignedPrimaryIps,
					{
