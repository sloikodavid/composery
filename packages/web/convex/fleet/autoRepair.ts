...
export const sweepBoxHealth = internalAction({
	...
	handler: async (ctx) => {
		const boxIds: Id<"boxes">[] = await ctx.runQuery(
			internal.fleet.autoRepair.sweptBoxes,
			{}
		);
		...
		for (const boxId of boxIds) {
			try {
				const { reachable } = await ctx.runAction(
					internal.fleet.health.probeRuntime,
					{ boxId }
				);
				await ctx.runMutation(internal.fleet.autoRepair.recordProbe, {
					boxId,
					reachable
				});
				...
				const facts = await ctx.runQuery(
					internal.fleet.autoRepair.autoRepairFacts,
					{ boxId }
				);
				if (!facts) continue;
				...
				if (!decision.repair) {
					if (decision.reason === "attempt_limit_reached") {
						await ctx.runMutation(
							internal.fleet.autoRepair.alertRepairsExhausted,
