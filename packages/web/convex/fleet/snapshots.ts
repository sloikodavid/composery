...
export const runDelete = internalAction({
	...
	handler: async (ctx, args) => {
		...
		await ctx.runMutation(internal.fleet.snapshots.removeSnapshotRow, {
			snapshotRowId: args.snapshotRowId
		});
	}
});
	...
	handler: async (ctx, args) => {
		...
		if (!page.isDone) {
			await ctx.scheduler.runAfter(
				0,
				internal.fleet.snapshots.cascadeDeleteBoxSnapshots,
				{
					boxId: args.boxId,
...
export const scheduleAutomaticSnapshots = internalAction({
	...
	handler: async (ctx, args): Promise<void> => {
		const page: {
			...
			isDone: boolean;
			page: Id<"boxes">[];
		} = await ctx.runQuery(internal.fleet.snapshots.runningBoxIdsPage, {
			cursor: args.cursor ?? null
		});
		...
		for (const [index, boxId] of page.page.entries()) {
			await ctx.scheduler.runAfter(
				...
				internal.fleet.snapshots.startAutomaticSnapshot,
				{ boxId }
			);
		}
