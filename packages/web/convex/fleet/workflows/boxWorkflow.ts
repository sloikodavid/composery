...
export async function startWorkflow(
	...
	return await workflow.start(ctx, workflowRef, args, {
		...
		onComplete: internal.fleet.lifecycle.finishBoxOperation,
		context
	});
}
...
>(config: {
	...
	return workflow.define({
		...
		handler: async (step, args) => {
			...
			await step.runMutation(internal.fleet.lifecycle.markOperationRunning, {
			...
			try {
				...
				await step.runMutation(internal.fleet.lifecycle.markOperationFailed, {
			...
			await step.runMutation(internal.fleet.lifecycle.settleOperation, {
				operationId: typedArgs.operationId
			});
		}
	});
