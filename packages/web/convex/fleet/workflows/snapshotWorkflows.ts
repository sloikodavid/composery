	...
	run: async (step, args) => {
		...
		try {
			const { imageId, actionId } = await step.runAction(
				internal.fleet.infra.hetznerVps.createSnapshotImage,
				{
			...
			await step.runMutation(internal.fleet.snapshots.markCreating, {
				snapshotRowId,
				imageId,
				actionId
			});
			...
			for (;;) {
				const action = await step.runAction(
					internal.fleet.infra.hetznerVps.getAction,
					{ actionId },
					{ retry: true }
				);
			...
			const image = await step.runAction(
				internal.fleet.infra.hetznerVps.getImage,
				{ imageId },
				{ retry: true }
			);
			await step.runMutation(internal.fleet.snapshots.completeSnapshot, {
				snapshotRowId,
				operationId: args.operationId,
				sizeBytes: snapshotSizeBytes(image.imageSizeGb)
			});
		} catch (error) {
			await step.runMutation(internal.fleet.snapshots.failSnapshot, {
				snapshotRowId,
...
export const restoreBox = defineBoxWorkflow({
	...
	run: async (step, args) => {
		...
		const target = await step.runQuery(
			internal.fleet.snapshots.snapshotRestoreTarget,
		...
		await step.runMutation(internal.fleet.snapshots.markRestoreSucceeded, {
			boxId: args.boxId,
			operationId: args.operationId,
			snapshotRowId: args.snapshotRowId
		});
