...
export const repairBox = defineBoxWorkflow({
	...
	run: async (step, args) => {
		...
		if (box.parking_volume_stage !== "restoring") {
			if (volumeId === undefined) {
				...
				await step.runAction(
					internal.fleet.infra.ssh.requireReachableHost,
					{ boxId: args.boxId },
					{ retry: true }
				);
				const { usedBytes } = await step.runAction(
					internal.fleet.infra.ssh.measureParkingUsage,
					{ boxId: args.boxId },
					{ retry: true }
				);
				const created = await step.runAction(
					internal.fleet.infra.hetznerVps.createParkingVolume,
				...
				await step.runMutation(internal.fleet.lifecycle.recordParkingVolume, {
					boxId: args.boxId,
					volumeId
				});
			}
			await step.runAction(
				internal.fleet.infra.hetznerVps.attachParkingVolume,
				{ volumeId, serverId },
				{ retry: true }
			);
			await step.runAction(
				internal.fleet.infra.ssh.copyToParking,
			...
			await step.runAction(
				internal.fleet.infra.ssh.verifyParkingCopy,
				{ boxId: args.boxId, volumeId },
				{ retry: true }
			);
			// The one-way gate: only now is the volume authoritative and the
			// destructive rebuild permitted.
			await step.runMutation(internal.fleet.lifecycle.markParkingRestoring, {
				boxId: args.boxId
			});
		}
		...
		await step.runAction(
			internal.fleet.infra.hetznerVps.detachParkingVolume,
			{ volumeId },
			{ retry: true }
		);
		...
		await step.runAction(
			internal.fleet.infra.ssh.copyFromParking,
			{ boxId: args.boxId, volumeId },
			{ retry: true }
		);
		await step.runAction(
			internal.fleet.infra.ssh.verifyParkingBack,
			{ boxId: args.boxId, volumeId },
			{ retry: true }
		);
		...
		await step.runAction(
			internal.fleet.infra.ssh.unmountParking,
			{ boxId: args.boxId },
			{ retry: true }
		);
		await step.runAction(
		...
		await step.runAction(
			internal.fleet.infra.hetznerVps.deleteParkingVolume,
			{ volumeId },
			{ retry: true }
		);
		await step.runMutation(internal.fleet.lifecycle.clearParkingVolume, {
			boxId: args.boxId
		});
		...
		await step.runAction(
			internal.fleet.infra.ssh.repairRuntime,
			{ boxId: args.boxId },
			{ retry: true }
		);
		await step.runMutation(internal.fleet.lifecycle.markRepairSucceeded, {
			boxId: args.boxId,
