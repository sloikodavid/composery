...
export const markDeleted = internalMutation({
	...
	handler: async (ctx, args) => {
		...
		await ctx.scheduler.runAfter(0, internal.fleet.cleanup.deleteRuntimeData, {
			boxId: box._id
		});
		await ctx.scheduler.runAfter(0, internal.fleet.cleanup.sanitizeOperations, {
			boxId: box._id
		});
		...
		await ctx.runMutation(internal.fleet.snapshots.cascadeDeleteBoxSnapshots, {
			boxId: box._id
		});
