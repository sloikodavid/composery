...
import { renderComposeryEnv } from "./infra/runtimeArtifacts";
...
describe("normalizeRuntimeConfig", () => {
	...
	it("refuses a key that is not configurable", () => {
		expect(() =>
	...
	// on the box.
	it("stores booleans in the spelling the box actually reads", () => {
		expect(
	...
	it("bounds numbers and rejects non-integers", () => {
	...
	it("accepts only the log levels the editor understands", () => {
	...
	it("refuses values that could break out of an env-file line", () => {
		expect(() =>
			normalizeRuntimeConfig({
	...
	it("treats an empty optional value as unset rather than as an empty string", () => {
	...
	it("has no duplicate keys and never offers a managed variable", () => {
...
describe("applySecretIntent", () => {
	...
	it("keeps a secret the page did not submit", () => {
		expect(
			applySecretIntent({
	...
	// removes it.
	it("clears a secret submitted empty", () => {
		expect(
			applySecretIntent({
				normalized: {},
				stored,
	...
	it("replaces a secret submitted with a new value", () => {
		expect(
			applySecretIntent({
	...
	it("has nothing to restore for a box that never set one", () => {
		expect(
			applySecretIntent({
	...
	it("never restores a non-secret the page left out", () => {
		expect(
			applySecretIntent({
				normalized: {},
				stored,
	...
	it("derives its secret set from the fields, and it is not empty", () => {
...
describe("renderComposeryEnv with owner configuration", () => {
	...
	it("writes the owner's variables alongside the managed ones", () => {
	...
	it("renders a stable file for the same configuration", () => {
	...
	it("refuses to render a configuration that shadows a managed variable", () => {
		expect(() =>
			renderComposeryEnv({
				...managed,
	...
	it("still renders without any owner configuration", () => {
