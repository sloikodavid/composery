...
import { standingNotices } from "./runtime-update";
...
describe("standingNotices", () => {
	...
	it("reports the floor on a box that is otherwise on the cached release", () => {
	...
	it("reports the floor and the available update together", () => {
	...
	it("never claims a box is current when it cannot be compared", () => {
	...
	it("names no version it does not have", () => {
	...
	it("names the deadline when the floor has one, and says so when it has none", () => {
