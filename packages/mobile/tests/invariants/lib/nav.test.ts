// Every screen has a declared exit. The screens exist as files under `src/app/`
// and again as entries in `lib/nav.ts`, and the second cannot be generated from
// the first: what a given screen does on back is a decision per screen, not a
// fact derivable from its path. What is derivable is whether one was forgotten,
// which is all this checks - a screen with no entry falls through to the
// platform default and looks fine until someone presses back on it.
