...
- Add research conversations or sessions with an AI assistant to `docs/sessions/` with `bun run session:import <source>`, where the source is a ChatGPT share URL or a Claude Code session ID. When no reader supports the source, add one in `scripts/session/` instead of copying the session by hand.
