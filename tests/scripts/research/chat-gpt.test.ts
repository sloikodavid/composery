...
test("reads a conversation whose streamed payload is several megabytes long", async () => {
	const conversation = await readChatGptShare(
		`http://127.0.0.1:${server.port}/share/long`,
	);
	expect(conversation.title).toBe("A long conversation");
	expect(conversation.createdAt).toEqual(
		new Date(createdAtSeconds * millisecondsPerSecond),
	);
	expect(conversation.turns).toEqual([
		{ role: "user", text: "How long can an answer be?" },
