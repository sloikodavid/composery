...
const HANDLES_TS = new URL(
	"../packages/ide/overlay/lib/vscode/src/vs/base/browser/touchSelectionHandles.ts",
	import.meta.url
);
...
describe("touch selection handles", () => {
	...
	test("a drag beyond the pane still lands inside it", async () => {
		...
		expect(host.points[0][1]).toBeLessThan(VIEWPORT.bottom);
		expect(host.points[0][1]).toBeGreaterThan(VIEWPORT.top);
	});
