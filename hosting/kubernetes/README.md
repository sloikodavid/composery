...
A single replica, a `PersistentVolumeClaim` mounted at `/data`, a `Service`, and an
example `Ingress` for TLS. Composery is not horizontally scalable: `persistd` is a
single writer for one root-filesystem delta, so the Deployment is pinned to one replica
with the `Recreate` strategy.
...
Wait for the pod to become ready (the startup probe allows ~150s for `persistd apply`):

```bash
kubectl rollout status deploy/composery
```

Then browse to the Ingress host. Register the initial password in the browser, or
provide it from a Secret (see the commented `PASSWORD` block in `composery.yaml`):
...
The manifests are provider-neutral. Only the StorageClass and the edge differ - confirm
your cluster's classes with `kubectl get storageclass`:
...
`Service` of `type: LoadBalancer` instead of the example Ingress, and terminate TLS at
the load balancer or with cert-manager.
