...
The runtime env file is mounted at `/etc/composery/composery.env`. Set
code-server variables directly in that file when you need them; for example,
set `PASSWORD` to use an environment-managed password instead of first-visit
registration. Quote values containing `$`, such as `HASHED_PASSWORD`.
