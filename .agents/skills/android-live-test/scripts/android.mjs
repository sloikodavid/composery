...
const sdk = process.env.ANDROID_HOME ?? process.env.ANDROID_SDK_ROOT ?? resolve(home, "AppData/Local/Android/Sdk");
...
switch (command) {
  case "status": {
    ...
    try {
      ...
      process.stdout.write(runAdb(["shell", "dumpsys", "activity", "activities" ]).split("\n").filter((line) => line.includes("mResumedActivity")).join("\n") + "\n");
    } catch {}
    break;
  }
  case "boot": {
    ...
    const child = spawn(emulator, ["-avd", avd, "-no-snapshot-save"], { detached: true, stdio: "ignore", windowsHide: true });
    child.unref();
    console.log(`Started ${avd}. Run status until it reports device.`);
    break;
  }
  case "pair": process.stdout.write(run(adb, ["pair", required(args[0], "pair <host:port> <code>"), required(args[1], "pair <host:port> <code>")])); break;
  case "connect": process.stdout.write(run(adb, ["connect", required(args[0], "connect <host:port>")])); break;
  case "screenshot": writeFileSync(outputPath(args[0]), runAdb(["exec-out", "screencap", "-p"], { binary: true })); break;
  case "dump": {
    const path = outputPath(args[0]);
    runAdb(["shell", "uiautomator", "dump", "/sdcard/window.xml"]);
    writeFileSync(path, runAdb(["exec-out", "cat", "/sdcard/window.xml"], { binary: true }));
    break;
  }
  case "tap": runAdb(["shell", "input", "tap", required(args[0], "tap <x> <y>"), required(args[1], "tap <x> <y>")]); break;
  case "swipe": runAdb(["shell", "input", "swipe", ...args.slice(0, 5)]); break;
  case "text": runAdb(["shell", "input", "text", required(args.join(" "), "text <value>").replaceAll(" ", "%s")]); break;
  case "key": runAdb(["shell", "input", "keyevent", required(args[0], "key <name|code>")]); break;
  case "rotate": {
  ...
  case "open-url": runAdb(["shell", "am", "start", "-a", "android.intent.action.VIEW", "-d", required(args[0], "open-url <url>")], { stdio: "inherit" }); break;
  case "start": {
    const pkg = required(args[0], "start <package> [activity]");
    const startArgs = args[1] ? ["shell", "am", "start", "-n", `${pkg}/${args[1]}`] : ["shell", "monkey", "-p", pkg, "-c", "android.intent.category.LAUNCHER", "1"];
    runAdb(startArgs, { stdio: "inherit" });
    break;
  }
  case "stop": runAdb(["shell", "am", "force-stop", required(args[0], "stop <package>")]); break;
  case "reload": runAdb(["shell", "input", "keyevent", "KEYCODE_R", "KEYCODE_R"]); break;
  case "logcat": process.stdout.write(runAdb(["logcat", "-d", "-t", args[0] ?? "400"])); break;
  case "record": {
    const path = outputPath(args[0]);
    const seconds = String(Math.min(Number(args[1] ?? 15), 180));
    runAdb(["shell", "screenrecord", "--time-limit", seconds, "/sdcard/composery-record.mp4"], { stdio: "inherit" });
    writeFileSync(path, runAdb(["exec-out", "cat", "/sdcard/composery-record.mp4"], { binary: true }));
    break;
  }
