...
| `COMPOSERY_PERSISTENCE`                                  | Selects the persistence engine: `auto` (default), `overlay`, or `copy`. `copy` is the universal engine and works on every target; `auto` resolves to it today. `overlay` names the kernel-maintained engine (requires `CAP_SYS_ADMIN`); it is not yet available and an explicit `overlay` currently stops the container. An unsupported value stops the container with exit 64. See [Persistence](persistence.md#engines). |
