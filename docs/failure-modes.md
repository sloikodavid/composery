...
Composery's own way in is the one part no provider can report, so it is looked at on a schedule as well as whenever somebody uses it: one connection every six hours, which both proves the key still works and brings back the address the server answers on. A server that cannot answer is unknown, never missing, because a machine that is off has said nothing about our key.
