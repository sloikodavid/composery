...
All variables are listed in `.env.convex.example`. They are optional at deployment time so an environment can run without provisioning. Creating a server requires the token, controller identifier, firewall ID, locations, `SSH_ACCESS_ENCRYPTION_KEY`, and both quotas.

Set `SSH_ACCESS_ENCRYPTION_KEY` to 32 cryptographically random bytes encoded as base64. Pipe the value into `convex env set SSH_ACCESS_ENCRYPTION_KEY` without displaying it. Keep a secure backup; replacing or losing this key makes the stored SSH access secrets unreadable. Do not rotate it by overwriting the environment variable while allocations hold secrets encrypted with it. Each allocation has a separate Ed25519 management key, encrypted with AES-256-GCM and bound to its allocation ID.
