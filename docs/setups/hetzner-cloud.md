...
6. Push the backend to the intended deployment. Use internal `quotas:set` with kind `server`, no `userId`, and a limit at or below the project's server limit, which Hetzner sets for each project and does not report through the API. Request an increase from Hetzner support before raising it. Then use internal `quotas:set` again with a local `userId`, kind `server`, and a limit. Both start at zero, so nothing is created until they are set, and a zero limit stops new servers without deleting existing ones.
...
The initial Ubuntu image must include Python 3 and cloud-init. The SSH management scripts currently require a POSIX shell and OpenSSH's `sshd`; they discover executable paths from the server's `PATH`, pair effective settings with native debug assignments when quoted file boundaries matter, and report this feature as unsupported when a required tool, setting, or boundary is absent or ambiguous. Allow outbound HTTPS to the deployment's `CONVEX_SITE_URL`. Cloud-init installs the public management key for root, generates the native host keys, and reports the configured Ed25519 host key to `/ssh/host-keys`. A provisioning-delivered token authenticates the report, expires after 15 minutes, and cannot replace an already registered host key outside an explicit renewal window. The report retries for a bounded period. Provider running state does not establish that registration or SSH login succeeded. Callback failure leaves the previous management key usable and does not fall back to trusting a network-observed key. Cloud-init and provider metadata can retain the expired bootstrap token; they never receive the management private key. A customer may choose Windows or another guest OS; this current bootstrap surface does not manage its SSH command path.
